...
void TestMAPlotDataCreator::setUp() 
{
  ;
}

void TestMAPlotDataCreator::tearDown()
{
  m_ctx.dump(std::cerr);
}
...
void TestMAPlotDataCreator::test1()
{
  ...
  MAPlotDataCreator creator(m_ctx);
  ...
  CPPUNIT_ASSERT(plotData->getLabel() == "Generic 2-day MA");
  ...
  CPPUNIT_ASSERT_EQUAL(3, int(dataSegment->size()));
