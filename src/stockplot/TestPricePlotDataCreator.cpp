...
void TestPricePlotDataCreator::setUp() 
{
  ;
}

void TestPricePlotDataCreator::tearDown()
{
  m_ctx.dump(std::cerr);
}
...
void TestPricePlotDataCreator::test1()
{
  ...
  PricePlotDataCreator creator(m_ctx);
  ...
  CPPUNIT_ASSERT(PlotData::STYLE_lines == plotData->getStyle());
  CPPUNIT_ASSERT(plotData->getLabel() == "Generic Price");
  ...
  CPPUNIT_ASSERT_DOUBLES_EQUAL(1.00, (*dataSegment)[0].value, delta);
  CPPUNIT_ASSERT_EQUAL(boost::posix_time::from_iso_string("19990131T235959"),
                       (*dataSegment)[0].timestamp);


  CPPUNIT_ASSERT_DOUBLES_EQUAL(0.70, (*dataSegment)[1].value, delta);
  CPPUNIT_ASSERT_EQUAL(boost::posix_time::from_iso_string("20000131T235959"),
                       (*dataSegment)[1].timestamp);


  CPPUNIT_ASSERT_DOUBLES_EQUAL(1.10, (*dataSegment)[2].value, delta);
  CPPUNIT_ASSERT_EQUAL(boost::posix_time::from_iso_string("20010131T235959"),
                       (*dataSegment)[2].timestamp);


  CPPUNIT_ASSERT_DOUBLES_EQUAL(0.60, (*dataSegment)[3].value, delta);
  CPPUNIT_ASSERT_EQUAL(boost::posix_time::from_iso_string("20020131T235959"),
                       (*dataSegment)[3].timestamp);
